...
methods can be divided into problem transformation methods and algorithm adaption methods.

At this moment the only problem transformation method, that is implemented in [%mlr]
is the binary relevance method. Integrating further of these methods require good knowledge 
of the architecture of the [%mlr] package.

The integration of an algorithm adaption multilabel-classification learner is easier and works very similar to the normal multiclass-classification. 
In contrast to the multiclass case, not all of the multiclass-classification properties are relevant. In particular, whether one-, two-, or multi-class problems are supported is not applicable. 
...
Below is the definition for the [rFerns](&rFerns::rFerns) learner from the
