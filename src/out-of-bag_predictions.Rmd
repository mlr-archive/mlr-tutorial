...
Some learners like random forest use bagging. Bagging means that the learner consists of a ensemble of several base learners and each base learner is trained with a different random subsample or bootstrap sample from all observations. For predicting an observation, 
that was used in the training process, one can predict the observation with all base learners, where this observation was not used 
for training, because it was not sampled. These predictions are called out-of-bag (OOB) predictions and are not prone to 
