...
Note that if you are using parallel computing, you probably need to use 
`set.seed(123, "L'Ecuyer")` in order to ensure the results are reproducible for 
each child process. See the documentation for [mccollect](&parallel:mccollect) 
for more information on reproducibility and parallel computing.
...
It returns a [list](&base::list) of lists of [WrappedModel](&makeWrappedModel) objects.
...
In this case you can perform an additional benchmark experiment and then merge the results
to get a single [&BenchmarkResult] object that can be accessed and analyzed as usual.

[%mlr] provides two functions to merge results:
[&mergeBenchmarkResultLearner] combines two or more benchmark results for different sets of
learners on the same [&Task]s, while [&mergeBenchmarkResultTask] fuses results obtained
with the same [Learner](&makeLearner)s on different sets of [&Task]s.
...
and [quadratic discriminant analysis (qda)](&MASS::qda) and use [&mergeBenchmarkResultLearner]
...
Moreover, in the second plot we color the boxes according to the learners to make them better
distinguishable.
...
Note that by default the measure `name`s are used as labels for the y-axis.
...
If you prefer the shorter `id`s like mmce and ber set `pretty.names = FALSE` (as done for
the second plot).
Of course you can also use the [ylab](&ggplot2::labs) function to choose a completely
different label.

Another thing which probably comes up quite often is changing the panel headers (which default
to the [&Task] IDs) and the learner names on the x-axis (which default to the
[Learner](&makeLearner) IDs).
...
Moreover, compared to the other learner names "randomForest" seems a little long.
...
levels(plt$data$learner.id) = c("lda", "rpart", "rF")
