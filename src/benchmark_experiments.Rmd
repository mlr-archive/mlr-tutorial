...
In this example the mean misclassification error ([mmce](measures.md)), the balanced error rate ([ber](measures.md)) and accuracy ([acc](measures.md)) are calculated.
...
By evaluating at the result we realize that our [random Forest](&randomForest::randomForest) outperforms the  [partition tree](&rpart::rpart) in every instance, while  [linear discriminant analysis](&MASS::lda) performs better then [rpart](&rpart::rpart) most of the time. Additionally [lda](&MASS::lda) sometimes even beats the random forest implementation.
...
The data generated by such data generation functions contains all info that is necessary for plotting later on.  
...
Although [BenchmarkResults](&benchmark) usually provide a variety of measures, subsequent analysis is best performed on one or few selected measures. In this case [mean misclassification error](&measures.md) is chosen as it is easy to compute and understand. In order to break down the benchmark result, we can either display a [rank matrix](&convertBMRToRankMatrix) or plot it as a [bar plot](&plotRankMatrixAsBar).  
...
Many researchers feel the need to display an algorithms superiority by employing some sort of hypothesis testing. As non-parametric tests seem better suited for such benchmark results the tests provided in [%mlr] are the **Overall Friedman test** and the **Friedman-Nemenyi post hoc test**.

While the ad hoc [Friedman test](&friedmanTestBMR) based on [friedman.test](&friedman.test) from the base package is testing the hypothesis whether there is a significant difference between the employed learners, the post hoc [Friedman-Nemenyi](&friedmanPostHocTestBMR) tests for significant differences between all pairs of learners. *Non parametric* tests often do have less power then their *parametric* counterparts but less assumptions about underlying distributions have to be made. This often means many **data sets** are needed in order to be able to show significant differences at reasonable significance levels. 
...
At this level of sigfnificance, we can accept the hypothesis that there exists a significant difference between the  decision tree(`classif.rpart`) and the  [random Forest](&randomForest::randomForest).   
...
df = reshape2::melt(perf, id.vars = c("task.id", "learner.id", "iter"), measure.vars = c("acc", "mmce", "ber"))
df= df[df$variable != "acc",]
head(df)

qplot(variable, value, data = df, colour = learner.id, facets = . ~ task.id, geom = "boxplot",
...
matrix of learner mean misclassification errors ([mmce](&measures.md)) on the sonar data set.
