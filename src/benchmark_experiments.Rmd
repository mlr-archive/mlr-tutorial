...
[%mlr] offers several possibilities to analyze the results of a benchmark experiment.
...
### Integrated plots
Plots are produced using [%ggplot2], as this package enables further customization,
such as renaming plot elements or changing colors.


#### Visualizing performances
[&plotBMRBoxplots] creates box or violin plots for a [&BenchmarkResult] which show the
...
Below are both variants, box plots and violin plots. The first plot shows the [mmce](measures.md)
...
[aesthetics](&ggplot2::aes) to make them better distinguishable.

```{r, fig.width=9}
plotBMRBoxplots(bmr, measure = mmce, facet.wrap.nrow = 2)
plotBMRBoxplots(bmr, measure = ber, style = "violin", facet.wrap.nrow = 2) +
