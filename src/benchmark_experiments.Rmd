...
plotBMRBoxplots(bmr, measure = ber, style = "violin", pretty.names = FALSE, order.lrn = getBMRLearnerIds(bmr)) +
...
```{r, fig.asp = 7/9}
plt = plotBMRBoxplots(bmr, measure = mmce, order.lrn = learner.ids)
head(plt$data)
...
```{r, fig.asp = 7/9}
plotBMRRanksAsBarChart(bmr, pos = "tile", order.lrns = learner.ids)
```
...
plotBMRRanksAsBarChart(bmr, order.lrn = learner.ids)
plotBMRRanksAsBarChart(bmr, pos = "dodge", order.lrn = learner.ids)
```
