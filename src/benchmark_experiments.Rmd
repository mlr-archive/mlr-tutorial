...
Many researchers feel the need to display an algorithms superiority employing some sort of hypothesis testing. As nonparametric tests seem better suited for such benchmark results the tests provided in [%mlr] are the **Overall Friedman test** and the **Friedman-Nemenyi post hoc test**.

While the ad hoc [friedman test](&friedmanTestBMR) based on [friedman.test](&friedman.test) from the base package is testing the hypothesis wether there is a significant difference between the employed learners, the post hoc [friedman-nemenyi](&friedmanPostHocTestBMR) tests for significant differences between all pairs of learners. *Non parametric* tests often do not have much power, which means usually many **data sets** are needed in order to be able to show significant differences at reasonable levels. 
...
The `test = nemenyi` test compares all [learners](&makeLearner) to each other, thus the output are groups of not significantly different learners, i.e the mean ranks do not differ by a large enough amout, while the `test = "bd"` test performs an *pairwise comparison with a baseline*.  
```{r, dpi= 500}
# Nemenyi test
