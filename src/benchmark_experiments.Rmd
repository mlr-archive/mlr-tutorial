...
As comparing performance on only one task does not provide a generally valid answer, the comparison is performed on several tasks, using [mmce](measures.md) as a main performance measure.
...
By just looking at the result we realize that our [random Forest](&randomForest::randomForest) outperforms the  [partition tree](&rpart::rpart) in every instance, while  [lda](&MASS::lda) performs better then [rpart](&rpart::rpart) most of the time. Additionally [lda](&MASS::lda) sometimes even beats the random forest implementation.
...
As a first result, we might want to compare [learner](&makeLearner) performance across [Resample](&makeResampleDesc) iterations, in order to get an insight into how the individual [Learner](&makeLearner)s perform within the different tasks. Additionally we color the boxes according to the learner in order to facilitate distinction by adding additional `aesthetics`: `aes(color = learner.id)`.  
```{r}
...
This plot displays the [learner](&makeLearner)s performance in comparison to the best or worst performing learner within this task. It facilitates finding tasks where learners perform similarly.
...
Many researchers feel the need to display an algorithm's superiority employing some sort of hypothesis testing. As nonparametric tests seem better suited for such benchmark results the tests provided in [%mlr] are the **Overall Friedman test** and the **Friedman-Nemenyi post hoc test**.

While the ad hoc [friedman test](&friedmanTestBMR) based on [friedman.test](&friedman.test) from the base package is testing the hypothesis of whether there is a significant difference between the employed learners, the post hoc [friedman-nemenyi](&friedmanPostHocTestBMR) tests for significant differences between all pairs of learners. *Non parametric* tests often do not have much power, which means usually many **data sets** are needed in order to be able to show significant differences at reasonable levels. 
...
As we can reject the null hypothesis at a reasonable significance level we might now wanna test where these differences lie exactly. In order to keep the computational time for this tutorial small, the [learners](&makeLearner) are only evaluated on 6 tasks. Thus we use a significance level $\alpha = 0.1$.  
```{r}
friedmanPostHocTestBMR(res, p.value = 0.1)
```

At this level of significance, we can accept the hypothesis that there exists a significant difference between the  decision tree(`classif.rpart`) and the  [random Forest](&randomForest::randomForest). 
In order to visualize this, a [Critical differences diagram](&plotCritDifferences) can be plotted, using either the 
`test = nemenyi` or the Bonferroni-Dunn (`test = "bd"`) test.  
The `test = nemenyi` test compares all [learners](&makeLearner) to each other, thus the output are groups of not significantly different learners, i.e the mean ranks do not differ by a large enough amount, while the `test = "bd"` test performs an *pairwise comparison with a baseline*.  
```{r, dpi= 500}
# Nemenyi test
