...
You can use functions [getMeasureProperties](&MeasureProperties) and
[hasMeasureProperties](&MeasureProperties) to determine the properties of a [Measure](&makeMeasure).
...
and `$worst` and if it wants to be minimized or maximized (`$minimize`) during [tuning](tune.md)
or [feature selection](feature_selection.md).
...
`predict = "both"` in [&makeResampleDesc]) the `perf.train` vector is empty.
