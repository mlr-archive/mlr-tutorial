...
[CPO](&mlrCPO::CPO) objects have hyperparameters that can be adjusted at creation, or later using `[setHyperPars()](&ParamHelpers::setHyperPars)`. They are shown by the
...
The `[ParamSet](&ParamHelpers::ParamSet)` of a [CPO](&mlrCPO::CPO) can be inspected using `[&getParamSet]`, but it is also shown when verbosely printing a [CPO](&mlrCPO::CPO) using `!`.
```{r}
...
These hyperparameters even survive CPO composition and attachment to [&Learner]s:
```{r}
cpo = cpoScale() %>>% cpoPca()
...
For example, if training entails performing PCA, then for prediction, the data must not undergo another PCA, instead it needs
...
Learner object, see the following section. The `[CPOTrained](&mlrCPO::CPOTrained)` objects are explained in more detail in the mlrCP vignette.
...
using `[getCPOTrainedState()](&mlrCPO::getCPOTrainedState)`. The following example retrieves the PCA rotation matrix trained when fitting a `[CPOLearner](&mlrCPO::CPOLearner)` to `[&iris.task]`.
...
Under certain circumstances it can be useful to represent the operation of no preprocessing. This is done using the `[NULLCPO](&mlrCPO::NULLCPO)` object. If it is applied to data, attached to a Learner or composed with another [CPO](&mlrCPO::CPO), the result is not modified.
...
This makes it possible to tune over many different [CPO](&mlrCPO::CPO) configurations at once.

### CBind CPO
`cbind` other [CPO](&mlrCPO::CPO)s as operation. `[cpoCbind](&mlrCPO::cpoCbind)` makes it possible to build [CPO](&mlrCPO::CPO)s that perform different operations on data and paste the results next to each other.
