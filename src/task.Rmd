...
In multilabel classification, in contrast to traditional supervized classification,
each object can belong to more than one category at the same time.

In order to generate the [MultilabelTask](&Task) the input [data.frame](&base::data.frame) has
to contain feature columns and multiple target columns. Each target column corresponds to one
class label and has to be a logical vector that indicates for which training observations this
label is present.
