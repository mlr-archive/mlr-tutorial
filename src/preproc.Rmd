...
Some of the following simple methods to change a [&Task] (or [data.frame](&base::data.frame))
were already mentioned on the page about [learning tasks](task.md):
...
* [&joinClassLevels]: Only for classification: Merge existing classes to
  new, larger classes.
* [&mergeSmallFactorLevels]: Merge infrequent levels of factor features.
* [&normalizeFeatures]: Normalize features by different methods, e.g.,
...
* [&makePreprocWrapperLambert]: Wrapping learners inside of lambert $W\times F$ transforms
...
To this end [%mlr] provides two [wrappers](wrapper.md):

* [&makePreprocWrapperCaret] is an interface to all preprocessing options offered by [%caret]'s
  [preProcess](&caret::preProcess) function.
* [&makePreprocWrapper] permits to write your own custom preprocessing methods by defining
...
[&makePreprocWrapperCaret] is an interface to [%caret]'s [preProcess](&caret::preProcess)
...
For all possible options see the help page of function [preProcess](&caret::preProcess).

Note that the usage of [&makePreprocWrapperCaret] is slightly different than that of
[preProcess](&caret::preProcess).

* [&makePreprocWrapperCaret] takes (almost) the same formal arguments as [preProcess](&caret:preProcess),
  but their names are prefixed by `ppc.`.
* The only exception: [&makePreprocWrapperCaret] does not have a `method` argument. Instead
  all preprocessing options that would be passed to [preProcess](&caret::preProcess)'s `method`
  argument are given as individual logical parameters to [&makePreprocWrapperCaret].

For example the following call to [preProcess](&caret::preProcess)
```{r eval=FALSE}
preProcess(x, method = c("knnImpute", "pca"), pcaComp = 10)
```
with `x` being a [matrix](&base::matrix) or [data.frame](&base::data.frame)
would thus translate into
```{r eval=FALSE}
makePreprocWrapperCaret(learner, ppc.knnImpute = TRUE, ppc.pca = TRUE, ppc.pcaComp = 10)
```
where `learner` is a [%mlr] [Learner](&makeLearner) or the name of a learner class like
`"classif.lda"`.
...
By inspecting the underlying [qda](&MASS::qda) model, we see that the first 22
...
Below the performances of [qda](&MASS::qda) with and without PCA preprocessing are compared
in a [benchmark experiment](benchmark_experiments.md).
Note that we use stratified resampling to prevent errors in [qda](&MASS::qda) due to a too
...
Moreover, [qda](&MASS::qda) has several options that control how the class covariance matrices
or class probabilities are estimated.

Those preprocessing and learner parameters can be [tuned](tune.md) jointly.
Before doing this let's first get an overview of all the parameters of the wrapped learner
using function [&getParamSet].
...
are [qda](&MASS::qda) parameters.

Instead of tuning the PCA threshold (`ppc.thresh`) we tune the number of principal
components (`ppc.pcaComp`) directly.
Moreover, for [qda](&MASS::qda) we try two different ways to estimate the posterior probabilities
...
If the options offered by [&makePreprocWrapperCaret] are not enough, you can write your own
preprocessing wrapper using function [&makePreprocWrapper].

As described in the tutorial section about [wrapped learners](wrapper.md) wrappers are
...
* ``data`` is a [data.frame](&base::data.frame) with columns for all features and
  the target variable.
...
Regarding the latter note that the `center` and `scale` arguments of [scale](&base::scale)
...
* ``data`` is a [data.frame](&base::data.frame) containing *only* feature values
...
As already shown for the number of principal components in [&makePreprocWrapperCaret] we can
[tune](tune.md) them easily together with other hyperparameters of the learner.
...
at all.
In order to tune `center` and `scale` we have to add appropriate [LearnerParam](&ParamHelpers::LearnerParam)s
to the [parameter set](&ParamHelpers::ParamSet) of the wrapped learner.

As mentioned above [scale](&base::scale) allows for numeric and logical `center` and `scale`
...
be integrated into [%mlr] just [contact us](https://github.com/mlr-org/mlr/issues).
