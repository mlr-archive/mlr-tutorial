...
[ROCR](&ROCR::performance)) and also several generalizations of the AUC to the
multi-class case (e.g., [multiclass.au1p](measures.md), [multiclass.au1u](measures.md) based
...
1. Function [&plotROCCurves] can, based on the output of [&generateThreshVsPerfData],
   plot performance curves for any pair of [performance measures](measures.md) available in
   [%mlr].
...
Note that you can only use [learners](learner.md) that are capable of predicting probabilities.
Have a look at the [learner table in the Appendix](integrated_learners.md)
...
As you might recall [&generateThreshVsPerfData] calculates one or several performance measures
for a sequence of decision thresholds from 0 to 1.
It provides S3 methods for objects of class [&Prediction], [&ResampleResult]
and [&BenchmarkResult] (resulting from [predict](&predict.WrappedModel), [&resample]
...
Since we want to plot ROC curves we calculate the false and true positive rates ([fpr](measures.md)
and [tpr](measures.md)).
Additionally, we also compute error rates ([mmce](measures.md)).

```{r}
df = generateThreshVsPerfData(pred1, measures = list(fpr, tpr, mmce))
```

[&generateThreshVsPerfData] returns an object of class [ThreshVsPerfData](&generateThreshVsPerfData)
...
Additional to [linear discriminant analysis](&MASS::lda) we try a support vector machine
...
For this purpose just pass a named [list](&base::list) of [&Prediction]s to [&generateThreshVsPerfData].
...
[Benchmark Experiments](benchmark_experiments.md)) and, ideally, the support vector machine
should have been [tuned](tune.md).
...
The error rate ([mmce](measures.md)) for a threshold value of 0.5 is reported as well.
...
Calling [&generateThreshVsPerfData] and [&plotROCCurves] on the [benchmark result](&BenchmarkResult)
...
([test.mean](&aggregations)) for each threshold in the sequence.
...
a [%ROCR] [prediction](&ROCR::prediction-class) object, so you can easily generate
performance plots by doing steps 2. and 3. yourself.
[%ROCR]'s [plot](&ROCR::plot-methods) method has some nice features which are not (yet)
...
(Note that you have to use the measures provided by [%ROCR] listed [here](&ROCR::performance)
...
curves. Like [&generateThreshVsPerfData] it has S3 methods for objects of class [&Prediction],
