...
tsk = makeFDARegrTask(data = df, target = "V1", 
...
# Or simply resample instead
resample(lrn, tsk, resampling = cv3, measures =  mse)
```
...
taskW = convertFDATaskToNormalTask(task = tsk, method = "wavelets", pars = list(filter = "haar"))
print(taskW)
...
taskFa = convertFDATaskToNormalTask(task = taskCl, method = "fourier", 
  pars = list(fft.coeff = "amplitude"))
print(taskFa)
# ... or we prefer the phase:
taskFp = convertFDATaskToNormalTask(task = taskCl, method = "fourier", 
