...
df = data.frame("NIRObs1" = fuelSubset$NIR[1, ],
                "lambda" = fuelSubset$nir.lambda)
ggplot(df) + 
  geom_line(aes(y = NIRObs1, x = lambda))
```
...
In this case we simply want to extract the mean from each __UVVIS__ functional and the fourier transformed features from each __NIR__ functional.
...
feat.methods = list("NIR" = extractFDAWavelets(filter = "haar"))
task.w = extractFDAFeatures(task = tsk, feat.methods)

# Use the Daubechie wavelet with filter length 4.
feat.methods = list("NIR" = extractFDAWavelets(filter = "fd4"))
task.wd4 = extractFDAFeatures(task = tsk, feat.methods)
```
...
feat.methods = list("NIR" = extractFDAWavelets(fft.coeff = "amplitude"),
                    "UVVIS" = extractFDAWavelets(fft.coeff = "phase"))
task.fa = extractFDAFeatures(task = tsk, feat.methods)
task.fa
```
