...
[makeFunctionalData](&makeFunctionalData) returns a data.frame, where the functional features are contained as matricies.
...
* **Use a [learner](&Learner.md) suitable for functional data**
...
This essentially transforms the time signal to a time-scale representation, where every wavelet has a different resolution.
We can specify which additional parameters (i.e. the filter (type of wavelet) and the boundary) in the pars argument.
...
For more informations on wavelets consider the documentation [wavelets](dwt). A more comprehensive guide is for example given [here](https://www.cs.unm.edu/~williams/cs530/arfgtw.pdf).
...
This can be specified in the additional _trafo.coeff_ argument:
