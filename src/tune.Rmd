...
the RBF kernel parameter `sigma` of the [ksvm](&kernlab::ksvm) function.
...
In order to define a search space, we create a [ParamSet](&ParamHelpers::makeParamSet) object, which describes the
parameter space we wish to search. This is done via function [makeParamSet](&ParamHelpers::makeParamSet).
...
We could also define a search space from 10^-10 to 10^10 for both parameters through the use of `trafo` 
arg. Trafos work like this: All optimizers basically see the parameters on their
...
Many other parameters can be created, check out the examples in [makeParamSet](&ParamHelpers::makeParamSet).
...
This is the case with the `kpar` argument of [ksvm](&kernlab::ksvm) which is a list of kernel parameters like `sigma`. 
...
parameters to pass to the [ksvm](&kernlab::ksvm) learner. Optimization algorithms are considered 
[&TuneControl] objects in [%mlr].
...
the defaults, noting that we will have 4x4=16 combinations in the case of `discrete_ps`:
...
with the `resolution` arg. Here we change to 15 equal-sized steps in the space defined within the 
[ParamSet](&ParamHelpers::makeParamSet) object. For `num_ps`, this means 15 steps in the form of
...
Many other types of optimization algorithms are available. Check out [TuneControl](&mlr::TuneControl) 
for some examples. 
...
`maxit` arg controls the amount of iterations.
...
[&tuneParams]. We will use `discrete_ps` with grid search:
...
As no performance measure was specified, by default the error rate ([mmce](measures.md)) is
used.
...
If you are interested in optimizing several measures simultaneously have a look at [Advanced Tuning](advanced_tune.md)

In the example below we calculate the accuracy ([acc](measures.md)) instead of the error
rate. We use function [&setAggregation], as described in the section on [resampling](resample.md),
...
We can generate a [Learner](&makeLearner) with optimal hyperparameter settings
as follows:
...
Then you can proceed as usual. Here we refit and predict the learner on the complete [iris](&datasets::iris) data
set:
...
[generateHyperParsEffectData](&mlr::generateHyperParsEffectData):
...
In order to get the *transformed* parameter values instead, use the `trafo` arg:

```{r}
generateHyperParsEffectData(res, trafo = TRUE)
```

Note that we can also generate performance on the train data along with the validation/test data, as discussed in [Resampling](resample.md) aggregating measures section:

```{r}
rdesc2 = makeResampleDesc("CV", iters = 3L, predict = "both")
...
[plotHyperParsEffect](&mlr::generateHyperParsEffectData). In the example below, 
we plot the performance over iterations, using the `res` from the previous section:
...
For an in-depth exploration of generating hyperparameter tuning effects and plotting the data, check out [Hyperparameter Tuning Effects](hyperpar_tuning_effects.md)
...
A clean approach to ensure unbiased performance estimation is [nested resampling](nested_resampling.md),
