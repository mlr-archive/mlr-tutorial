...
parameter space we wish to search. This is done via the function [makeParamSet](&ParamHelpers::makeParamSet).
...
We could also define a continuous search space (using [makeNumericParam](&ParamHelpers::makeNumericParam) instead of [makeDiscreteParam](&ParamHelpers::makeDiscreteParam)) from 10^-10 to 10^10 for both parameters through the use of the `trafo` 
