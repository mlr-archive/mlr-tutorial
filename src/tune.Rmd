...
We could also define a continuous search space (using [makeNumericParam](&ParamHelpers::makeNumericParam)
instead of [makeDiscreteParam](&ParamHelpers::makeDiscreteParam)) from $10^{-10}$ to $10^{10}$ for
...
Of course, you can pass other measures and also a [list](&base::list) of measures to [&tuneParams].
...
rate. We use function [&setAggregation], as described on the [resampling](resample.md) page,
...
Note that the result of [generateHyperParsEffectData](&mlr::generateHyperParsEffectData)
...
check out [Hyperparameter Tuning Effects](hyperpar_tuning_effects.md).
