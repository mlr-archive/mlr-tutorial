...
First, let's take a look at the effect for a classification [task](task.md).
Based on a simulated [ClassifTask](&Task) with imbalanced classes two new
tasks (`task.over`, `task.under`) are created via [%mlr] functions
[&oversample] and [undersample](&oversample), respectively.
...
As a result the [performance](performance.md) should improve if the model is applied to new data.
...
As the *misclassification rate* ([mmce](measures.md)) evaluates the overall
accuracy of the predictions, the *balanced error rate* ([ber](measures.md)) and
*area under the ROC Curve* ([auc](measures.md)) 
...
Alternatively, [%mlr] also offers the integration of over- and undersampling
via a [wrapper approach](wrapper.md). This way
over- and undersampling can be applied to already existing [learners](learner.md)
to extend their functionality. 

The example given above is repeated once again, but this time with extended
learners instead of modified tasks (see [makeOversampleWrapper](&makeUndersampleWrapper)
...
the [bagging approach](bagging.md). For each iteration of the bagging process,
...
the [wrapper approach](wrapper.md).
