...
* Use a problem transformation method (for more information see our Paper):
...
Therefore a order of the labels has to be specified. At prediction time the labels are predicted in the same order as while training. 
The required labels in the input data are given by the previous done prediction of the respective label. 

..* Nested Stacking
Same as Classifier Chains, but the labels in the input data are not the real ones, but estimations of the labels obtained by the 
already trained learners. 

..* Dependent Binary Relevance
Each label is trained with the real observed values of all other labels. In prediction phase for a label the other necessary labels are 
obtained in a previous step by a base learner like the binary relevance method.

..* Stacking
Same as the dependent binary relevance method, but in the training phase the labels used as input for each label are obtained by the binary
relevance method. 

### Algorithm adaptation method
Currently the available algorithm adaptation methods in **R** are the multivariate random forest in the [%randomForestSRC] package and Random Ferns multilabel algorithm in the [%rFerns] package. You can create the learner for this algorithm
like in multiclass classification problems.

```{r}
multilabel.lrn.rfsrc = makeLearner("multilabel.randomForestSRC")
multilabel.lrn.rFerns = makeLearner("multilabel.rFerns")
multilabel.lrn.rFerns
```
...
[&makeMultilabelDBRWrapper], [&makeMultilabelStackingWrapper], on the learner to convert it to a learner that uses the respective problem transformation method.

You can also generate a binary relevance learner directly, as you can see in the example.

```{r}
multilabel.lrn.rpart = makeLearner("classif.rpart", predict.type = "prob")
multilabel.lrn.rpart = makeMultilabelBinaryRelevanceWrapper(multilabel.lrn)
multilabel.lrn.rpart

multilabel.lrn.rpart2 = makeMultilabelBinaryRelevanceWrapper("classif.rpart")
multilabel.lrn.rpart2
```
...
```{r}
mod = train(multilabel.lrn.rpart, yeast.task)
mod = train(multilabel.lrn.rpart, yeast.task, subset = 1:1500, weights = rep(1/1500, 1500))
mod

mod2 = train(multilabel.lrn.rfsrc, yeast.task, subset = 1:100)
mod2
```
...
performance(pred2, measures = list(subset01, hamloss, accmult, f1mult, timepredict))
...
r = resample(learner = multilabel.lrn.rpart, task = yeast.task, resampling = rdesc, show.info = FALSE)
r

r = resample(learner = multilabel.lrn.rfsrc, task = yeast.task, resampling = rdesc, show.info = FALSE)
r
```
