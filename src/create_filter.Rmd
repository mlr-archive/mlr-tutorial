...
A lot of feature filter methods are already integrated in [%mlr] and a complete list is given
in the [Appendix](filter_methods.md) or can be obtained using [&listFilterMethods].
...
## Filter objects
In [%mlr] all filter methods are objects of class [Filter](&makeFilter) and are registered
...
which interfaces function [rank.correlation](&FSelector::correlation) in package [%FSelector].
...
and passes them on to the [rank.correlation](&FSelector::correlation) function.

Additionally, each [Filter](&makeFilter) object has a `$name`, which should be short and is
...
You can integrate your own filter method using [&makeFilter]. This function generates a
[Filter](&makeFilter) object and also registers it in the `.FilterRegister` environment.

The arguments of [&makeFilter] correspond to the slot names of the [Filter](&makeFilter)
object above.
...
`fun` must be a [function](&base::function) with at least the following formal arguments:

* `task` is a [%mlr] learning [&Task].
* `nselect` corresponds to the argument of [&generateFilterValuesData] of the same name and
...
For writing `fun` many of the getter functions for [&Task]s come in handy,
...
You can use it like any other filter method already integrated in [%mlr] (i.e., via the
`method` argument of [&generateFilterValuesData] or the `fw.method` argument of
[&makeFilterWrapper]; see also the page on [feature selection](feature_selection.md)).
