...
To reduce this bias on the resulting predictive accuracy estimate, [Brenning 2005](https://www.nat-hazards-earth-syst-sci.net/5/853/2005/) suggested to use spatial partitioning in favor of random partitioning (see Figure 1). 
...
In any [resample](http://www.rdocumentation.org/packages/mlr/functions/resample.html) call you can choose `Spcross-validation` or `RepSpcross-validation` to use it. 
There are some prerequisites for this:

When specyfing the [task](task.md), you need to explicitly state that the task is `spatial` one by setting `spatial = TRUE`. 
Next, there need to be two variables named `x` and `y` in your data set that store the coordinate information. 
If this applies, the coordinates will be used for spatial partitining if `Spcross-validation` or `RepSpcross-validation` are selected as resampling strategies. 
...
The `basque` data set serves as an example data set for spatial modeling tasks in `mlr`.
...
resampling <- makeResampleDesc("SpRepcross-validation", fold = 5, reps = 10)
...
To force use non-spatial cross-validation here, variables `x` and `y` need to be removed from the data set because they are not used for parititiong when using `Repcross-validation` and also should not be used as predictors. 

Additionally, the `spatial` task attribute needs to be set to `FALSE`.
`mlr` tries to be smart by issuing a warning message if it detects variables named `x` or `y` in combination with  `spatial = FALSE`.
Finally, `Repcross-validation` is chosen instead of `SpRepcross-validation`.
...
resampling <- makeResampleDesc("Repcross-validation", fold = 5, reps = 10)
...
* The concept of spatial cross-validation was originally implemented in package [sperrorest](https://pat-s.github.io/sperrorest/index.html). This package comes with even more partitioning options and the ability to visualize the spatial grouping of folds. We plan to integrate more functions from [sperrorest](https://pat-s.github.io/sperrorest/index.html) into `mlr` so stay tuned!

* For more detailed information, see [Brenning 2005](https://www.nat-hazards-earth-syst-sci.net/5/853/2005/), [Brenning2012](http://ieeexplore.ieee.org/document/6352393/), Schratz2018 (in preperation).
