...
In any [&resample] call you can choose `Spcross-validation` or `RepSpcross-validation` to use it. 
...
If this applies, the coordinates will be used for spatial partitioning if `Spcross-validation` or `RepSpcross-validation` are selected as resampling strategies. 
...
To force use non-spatial cross-validation here, variables `x` and `y` need to be removed from the data set because they are not used for partitioning when using `Repcross-validation` and also should not be used as predictors. 
