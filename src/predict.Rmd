...
predicted values and the rows the true labels.
...
Since it is possible to normalize by row **or** column but not both at once, every element of the 
relative confusion matrix consists of two values. The first is the relative frequency grouped by row 
(the true label) and the second value grouped by column (the predicted label).

Additionally we can also already calculate the absolute number of observations for each predicted and true label. These can be added to the matrix (both absolute and relative) with `sums=TRUE`.
