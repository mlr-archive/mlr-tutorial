...
Function [&configureMlr] permits to set options globally for your current **R** session.
...
(Note that [multinom](&nnet::multinom) has a `trace` switch that can alternatively be used to turn off
the progress messages.)

To globally suppress the output for all subsequent learners and calls to [&resample],
[&benchmark], etc. try the following:
...
Function [&getMlrOptions] returns a [list](&base::list) with the current configuration.
...
It might happen that you want to access a new parameter of a [Learner](&makeLearner) which
is already available in [%mlr], but the parameter is not "registered" in the learner's
[parameter set](&ParamHelpers::makeParamSet) yet.
...
If an underlying learning method throws an error the default behavior of [%mlr] is to
...
Instead of an exception, a warning is issued and a [&FailureModel] is created.
