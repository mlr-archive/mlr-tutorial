...
head(BostonHousing)
```
We have 14 variables. As before, ``medv`` is chosen to be the response variable. 
...
A closer look at the first boxplot reveals that random forest outperforms the other learners for this specific task. However, the SVM model using the measure ``mae`` seems to perform better than the other models. In both cases the benchmark experiment for linear and lasso regression yields similar but poor results.
