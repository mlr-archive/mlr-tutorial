...
[%mlr] can estimate the partial dependence of a learned function on a subset of the feature space using
...
The first step executed by [&generatePartialDependenceData] is to generate a feature grid for
...
Results from [&generatePartialDependenceData] and [&generateFunctionalANOVAData] can be visualized with [&plotPartialDependence]
...
For regression tasks, when the `fun` argument of [&generatePartialDependenceData] is used,
...
When multiple features are passed to [&generatePartialDependenceData] but `interaction = FALSE`,
...
When `interaction = TRUE` in the call to [&generatePartialDependenceData], one variable must
...
[&plotPartialDependenceGGVIS] can be used similarly, however, since [%ggvis] currently lacks
