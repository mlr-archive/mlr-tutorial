...
For example, to access a [task description](&TaskDesc) you can use

```{r}
getTaskDescription(regr.task)
```

The most important operations are listed in the documentation of [&Task].
...
Note the many options that [&getTaskData] provides to convert the data set into a convenient format.
...
Some of these functions are explained in more detail in the [data preprocessing](preproc.md) section.


## Example tasks
For your convenience [%mlr] provides pre-defined [tasks](&Task) for each type of learning problem.
These are used throughout this tutorial in order to get shorter and more readable code.
A list of all [tasks](&Task) can be found [here](example_tasks.md).
